...
?>
                                         <div style="padding: 30px;background-color: whitesmoke;">
                                            <form method="post">
                                                <input type="text" placeholder="Ajouter un commentaire" name="comment" style="width: 400px;height: 40px;margin-right: 10px;border-radius: 10px;" required>
                                                <input type="submit" name="com" value="envoyé" class="env">
                                            </form>
                                        </div>
                </div>
                <?php
                            if(isset($_POST['com']))
                            {
                                mysqli_query($conn, "INSERT INTO commentaire(code_utl,code_art,content) VALUES($code_utl, $code_art, \"".$_POST['comment']."\")");
                                print("<script> window.location.replace('article.php?article=$code_art'); </script>");
                            }
                              ?>
